...
<layout:Default pageId="orderList">
	<h1>Order List</h1>
	<form:form action="${pageContext.request.contextPath}/yushan/list" 
		...
		<select name="condition.status" class="dropdown">
			...
			<option value="0" <c:if test="${view.condition.status ==0}">selected="true"</c:if>>Draft</option>
	...
	function editOrder(orderId){
		document.forms[0].action = "${pageContext.request.contextPath}/yushan/info";
	...
	function deleteOrders(){
		...
		if(_selected){
			if(confirm("你确定要删除这些记录？")){
				document.forms[0].action = "${pageContext.request.contextPath}/yushan/delete";
				document.forms[0].submit();	
			}
		}else{
			alert("需要至少选一条记录！");
	...
	function addOrder(){
		document.forms[0].action = "${pageContext.request.contextPath}/yushan/add";
		document.forms[0].submit();
	}
